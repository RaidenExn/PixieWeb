...
class WPSpin:
    ...
    def promptPin(self, bssid: str) -> Optional[str]:
        """(FOR CLI) Interactively prompts user to select a PIN."""
        pins = self._getSuggested(bssid)
        if not pins:
            return None

        if len(pins) > 1:
            print(f"PINs generated for {bssid}:")  # This print is for CLI
            ...
            while True:
                ...
                try:
                    ...
                    if 1 <= pin_no <= len(pins):
                        ...
                        print(
                            f"Invalid number. Please enter a number from 1 to {len(pins)}."
                        )
                except ValueError:
    ...
    def getLikely(self, bssid: str) -> Optional[str]:
        """(FOR WEB UI) Gets the most likely PIN without user interaction."""
    ...
    @staticmethod
    def _pin24(bssid: NetworkAddress) -> int:
        return bssid.INTEGER & 0xFFFFFF

    @staticmethod
    def _pin28(bssid: NetworkAddress) -> int:
        return bssid.INTEGER & 0xFFFFFFF

    @staticmethod
    def _pin32(bssid: NetworkAddress) -> int:
        return bssid.INTEGER % 0x100000000

    @staticmethod
    def _pinDLink(bssid: NetworkAddress) -> int:
    ...
    @staticmethod
    def _pinASUS(bssid: NetworkAddress) -> int:
    ...
    @staticmethod
    def _pinAirocon(bssid: NetworkAddress) -> int:
        b = [int(i, 16) for i in str(bssid).split(":")]
        pin = (
            ((b[0] + b[1]) % 10)
            + (((b[5] + b[0]) % 10) * 10)
    ...
    def _pinDLink1(self, bssid: NetworkAddress) -> int:
        bssid.INTEGER += 1
        return self._pinDLink(bssid)

    def _generate(self, algo: str, bssid: str) -> str:
        try:
    ...
    def _getSuggested(self, bssid: str) -> List[Dict[str, Any]]:
    ...
    def _getSuggestedList(self, bssid: str) -> List[str]:
        algos = self._suggest(bssid)
        res = []
        for algo in algos:
